...
'''
#matrice correlazione tra lidar e imu sbagliata -> aumento varianza lidar
#si riscontra che diminuendo la varianza del imu_w si riscontra una migliore stima del andamento 
var_imu_f = 0.20
var_imu_w = 0.01
var_gnss  = 0.01
var_lidar = 10000.00
...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    K_gain=p_cov_check.dot(h_jac.T).dot(np.linalg.inv(h_jac.dot(p_cov_check).dot(h_jac.T)+R))
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    #uso la funzione interna to_mat() di quaternion per ottenere la matrice di rotazione rispetto al quaternione 
    ...
    #si veda da slide la matrice di F
    F=np.eye(9)
    F[:3, 3:6] = delta_t * np.identity(3)
    F[3:6, 6:] = -(rotation_matrix.dot(skew_symmetric(imu_f.data[k-1].reshape((3,1)))))
    
    #matrice Q data dalla varianza
    vfa = var_imu_f**2
    vfw = var_imu_w**2
    Q = delta_t**2 * np.diag([vfa,vfa,vfa,vfw,vfw,vfw])
    ...
    #verifica della compatibilita temporale dei dati
    if gnss_i < 49:
        ...
        #applicazione in base alla validità dei dati
